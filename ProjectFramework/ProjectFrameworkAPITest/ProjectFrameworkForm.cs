    ...
    public partial class ProjectFrameworkForm : Form
    {
        ...
        private async void button1_Click(object sender, EventArgs e)
        {
            using (var client = new HttpClient())
            {
                ...
                HttpResponseMessage response = await client.GetAsync("api/Contact/GetSettingsInfo");
                string ResponseData = "";
                if (response.IsSuccessStatusCode)
                {
                    ResponseData = await response.Content.ReadAsStringAsync();

                    ContactSettings Settings = JsonConvert.DeserializeObject<ContactSettings>(ResponseData);
                    textBoxCoName.Text = Settings.ConName;
                    textBoxAdress.Text = Settings.Adress;
                    textBoxWeb.Text = Settings.Web;
                    textBoxNumber.Text = Settings.Number;
        ...
        private async void button2_Click(object sender, EventArgs e)
        {
            ...
            Settings.ConName = textBoxCoName.Text;
            Settings.Adress= textBoxAdress.Text;
            Settings.Web = textBoxWeb.Text;
            Settings.Number = textBoxNumber.Text;
